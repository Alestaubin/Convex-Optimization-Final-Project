...
% b = imnoise(b,'gaussian', mean, vargauss);
d = 0.5;
...
%%%%%%%%%%%%%%%%%%%%
function algo = douglasrachfordprimal()
...
apply = multiplyingmatrix(b, kernel, i);
ivals.z1 = xinit;
...
function apply = multiplyingmatrix(b, kernel, i)
...
apply.eigValsMat = ones(numRows, numCols) + t*t*eigArry_KTrans.*eigArry_K + t*t*eigArry_D1Trans.*eigArry_D1...
...
switch lower(name)
    case 'douglasrachfordprimal'
        algo = douglasrachfordprimal();
    case 'douglasrachfordprimaldual'
        algo = douglasrachfordprimaldual();
    case 'admm'
        algo = admm();
    case 'chambollepock'
        algo = chambollepock();
...
switch lower(name)
    case 'l1'
        problem =@(y, b, t) l1Prox( y, b, t );
    case 'l2'
        problem =@(y, b, t) l2Prox( y, b, t );
    otherwise
        error('Unknown algorithm name');
end
end
