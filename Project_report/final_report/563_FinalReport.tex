...
\subsection{Primal Douglas-Rachford splitting (Spingarn's method)}%
  ...
  \begin{equation}
    ...
    \end{split}
  \end{equation}
  We now focus on finding the resolvents of $A$ and $B$,  
  \begin{equation}
    \begin{split}
      J_A = (I + \lambda A)^{-1} &= \prox_{\lambda (f+g) }(x,y)\quad [\text{by (\ref{subProx})}] \\
  ...
  \begin{equation}
    \begin{split}
      \prox_{\lambda f} (x) =& \prox_{\lambda \delta_S}(x) = \argmin_x \frac{1}{2} \| u-x \|^2_2 + \delta_S(x)= \argmin_x \frac{1}{2} \| u-x \|^2_2,
    \end{split}
  \end{equation}
  which is the euclidean projection operator onto $S.$ Now, we observe that $g(y)$ described in  (\ref{gf}) can be written in a different form by letting $$y_1 = Ky\quad \text{ and }\quad (y_2,y_3) = Dy$$ to get
  \begin{equation}
    \begin{split}
      $g(y_1, y_2, y_3) = \| y_1 -b \|_1 + \gamma \|(y_2, y_3) \|_{\iso}$
    \end{split}
  \end{equation}
  Then, by Proposition \ref{proxsplit}, we notice that the proximal operator of $g$ may be obtained by splitting its components into 
  \begin{equation*}
    \begin{split}
  ...
  \begin{equation*}
    \begin{split}
      \prox_{\lambda \varphi} (y_1) = \prox_{\lambda \| \cdot - b \|_1}(y_1)
    \end{split}
  \end{equation*}
  \newpage
