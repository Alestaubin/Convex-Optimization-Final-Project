...
\subsection{Primal Douglas-Rachford splitting (Spingarn's method)}%
  ...
  \begin{equation}
    ...
    \end{split}
  \end{equation}
  As stated in the literature \cite{ConnorVander}, $F$ is the subdifferential of closed, convex functions, hence is a maximal monotone operator. Thus, by (\ref{maxmon}), the zeros of $F$ are the fixed points of the resolvent of $F$. Therefore, if we can find the fixed points of $F$, we'll have found the optimal points. Unfortunately, this particular problem is too computationally expensive, so the strategy is to split $F$ into two maximal monotone operators $\mathcal{A}$ and $\mathcal{B}$ which have inexpensive proximal operators. Finding the fixed points of $\mathcal{A}+ \mathcal{B}$ will be equivalent to finding that of $F.$ Define $\mathcal{A}$ and $\mathcal{B}$ as, 
  \begin{equation}
    \begin{split}
  ...
  \begin{equation}
    ...
    \end{split}
  \end{equation}
  which is the euclidean projection operator onto $S.$ Now, we observe that $g(y)$ described in  (\ref{gf}) can be written in a different form by letting  
  \begin{equation}
    \begin{split}
